...
@WebServlet(name = "RegistrationModeratorServlet", value = "/registration")
public class RegistrationModeratorServlet extends HttpServlet {

    private static final Logger logger = LogManager.getLogger(RegistrationModeratorServlet.class);
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        logger.info("RegistrationModeratorServlet: doGet method.");
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        logger.info("RegistrationModeratorServlet: doPost method.");
        HttpSession session = req.getSession();
        if (session.getAttribute("ex") != null) {
            session.removeAttribute("ex");
        }
        ...
        try {
            ...
            if (user != null) {
                if (user.getRoleId() == 2) {
                    logger.error("RegistrationSpeakerServlet: this email use speaker.");
                    session.setAttribute("ex", "This email use speaker.");
                    address = PathUtil.REGISTRATION_PAGE;
                } else if (user.getRoleId() == 3) {
                    userService.updateUser(extractUser(req, user));
                    address = URLUtil.LOGIN_URL;
                } else {
                    logger.error("RegistrationSpeakerServlet: there is moderator with this email. ");
                    session.setAttribute("ex", "There is moderator with this email.");
                    address = PathUtil.REGISTRATION_PAGE;
                }
            } else {
